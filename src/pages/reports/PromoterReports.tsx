...
import { usePromoterReports } from '@/hooks/useReports';
import { ROLES } from '@/lib/constants';
import { formatDate, getAxiosMessage } from '@/lib/utils';
import type { PromoterReport } from '@/types';
import { useQuery } from '@tanstack/react-query';
import { AlertTriangle, Eye } from 'lucide-react';
import { useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';

export function PromoterReports() {
  ...
  const outletsQ = useQuery({
    queryKey: ['outlets', 'promo-filters'],
  ...
  const columns: TableColumn<PromoterReport>[] = useMemo(
    () => [
      {
        key: 'date',
        header: 'Date',
      ...
      },
      {
        key: 'contacted',
        header: 'Contacted',
        render: (row) => row.personsContacted ?? '—',
      },
      {
        key: 'tasted',
        header: 'Tasted',
        render: (row) => row.personsTasted ?? '—',
      },
      {
        key: 'sales',
        header: 'Sales',
        sortable: true,
        render: (row) => row.sales.length,
      },
      {
        key: 'samples',
        header: 'Samples',
        render: (row) => row.samples.length,
      },
      {
        key: 'status',
        header: 'Status',
      ...
      },
      {
        ...
        render: (row) => (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              navigate(`/reports/promoter/${row.id}`);
            }}
          >
  ...
  return (
    <div className="space-y-6">
      <h1 className="sr-only">Promoter Reports</h1>
      ...
      {sorted.length === 0 ? (
        ...
        <>
          <Table
            ...
            onRowClick={(row) => navigate(`/reports/promoter/${row.id}`)}
          />
          <Pagination
            page={page}
            limit={limit}
